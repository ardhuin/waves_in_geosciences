...
but ripples already appear on the water surface for winds as low as  $U= 1.1$~m~s$^{-1}$ \citep{Kahma&Donelan1988}. 
...
This parameterization part of the  `WAM Cycle 3'  parameterizations \citep{WAMDI1988}. 
...
\end{equation}
If there are components $(\kb,\sigma)$ such that $\kb=\kb_1+\kb_2$ and $\sigma=\sigma_1+\sigma_2$ then the solution contain resonances. \cite{Phillips1960} showed
...
The actual Euler equation (see Part 3) has a few extra terms but it is the same principle with a similar result, with the only difference that $A$ is more complex. 
...
the second order has components with frequency $2f$ and 0. 
...
(1-\lambda)^2 k$. This DIA, as adjusted by \cite{Hasselmann&al.1985b}, uses  $\lambda=0.25$ and $\gamma
= \sqrt{2}$. 
