...
\subsection{Ice floes}
Solid slabs of ice with a wide range of horizontal dimensions (1 m to 100 km or more) can be formed from the freeze-up of many pancakes in calm conditions, or from the freeze-up of a quiet body of water. These slabs are called floes, and the patches of water in between floes are leads. Floe size is very important parameter when considering the interaction of waves and sea ice. Waves often define the diameter of the ice floes as ice can flex a little but the thicker it gets, the more prone it is to break. In return the break up of the ice layer completely changes the attenuation properties of the ice. Figure \ref{fig:S2_floes} shows some examples of floes from a few tens of meters to 1~km. More images can be viewed at \url{https://odl.bzh/6f6m7-4X}. When waves are high enough to break up the ice, the floe size distribution is controlled by the wavelengths and, in the left panel of Fig. \ref{fig:S2_floes}, the largest floe is less than 400 m across, and can be seen flexing in the waves (different shades of grey correspond to a different local slope of the ice surface exposed to the grazing sun). In the middle panel the largest floe (near $y=3~km$) exhibits a crack along the wave crest orientation and will probably soon split in two or more pieces. In the right panel, some floes are larger than the wavelenghts. When waves are too small in height, as occurs over most of the Arctic, the floe size are determined by other forces that produce ice deformation, mainly winds and currents, giving floes that can be much larger, especially for thick ice. 
