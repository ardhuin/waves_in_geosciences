...
If $\tau$ is large enough, then the frequency $1/(2 \tau)$ will fall in the region where $\Psi(f) \simeq \Psi(f=0)$. For the case shown in Fig. \ref{fig:groups1D}, that can be up to 0.02 or even 0.04~Hz. We can use this approximation to estimate the  variance of $\widehat{H}_\tau$ as
\begin{equation}
...
\end{equation}
 ...
 where $dk_x dk_y$ is the spectral resolution, corresponding to $dk_x dk_y=(2 \pi)^2/(2 L)^2$, with 
\begin{equation} 
...
% Note: : std / mean = 1/sqrt(2 k)  for chi. For chi 2: var / mean :  ...  sqrt( 1/2 k).= Qkk * 2*pi / L / sqrt(2).    OK. Now, with H^2 : var (H**2°/mean(H**2) = 2  ... std / mean  = sqrt(2/k) = Qkk * sqrt(2) *2*pi / L

These fluctuations in the estimate of wave heights are now easy to see with SWOT maps of wave heights, derived from the "volumetric decorrelation" in the KaRIN interferometer. 
They are most pronounced when very long swell is present. In the example shown in Fig. \ref{fig:groups_SWOT}.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{figure}[h!]
\centerline{\includegraphics[width=0.9\textwidth]{FIGS_CH_GROUPS/SWOT_groups_resolved.pdf}}
%\vspace{3.64in}
  \caption{Variability of "SWH" (in colour) around 0.7$^\circ$ N on track 26 of cyclz 26 (24 December 2024). Here SWH is estimated from two instruments on board the SWOT satellite: KaRIN  and Poseidon-3C with two different method \citep{Bohe&al.2025}. KaRIN also provides a map of the sea surface elevation, shown in grey scale, that resolves swells with periods longer than 18 s. The large values in the very patchy map of SWH correspond to groups of long waves. These long waves were generated by storm Eddie \citep{Ardhuin&al.2025}. More of this data can be viewed using the Ocean Virtual Laboratory, \href{https://odl.bzh/2GsA5tZQ}. }
\label{fig:groups_SWOT}
\end{figure}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
This shows local wave heights in colors with values given on a 2 km resolution grid, and estimated from the interferometric coherence over squares regions of 4.2 km. These estimates of $H_L$ that range from about 2 to 4 m.  Not only KaRIN is uniquely capable of measuring maps of wave heights \citep{Bohe&al.2025}, it is also capable of resolving the longest waves in its sea level measurement \citep{Ardhuin&al.2024}. We can thus directly verify the impact of the long wave groups on the measurement of wave height. 

Breaking up the wave spectrum in long waves, with periods longer than 18 s, and short waves, the SWH estimated from KaRIN is the root square sum of the short and long wave heights, 
\begin{equation}
H_L^2 = H_{L,T > 18 s}^2 + H_{L,T < 18 s}^2. \label{eq:HLsum}
\end{equation}
Fig. \ref{fig:groups_SWOT2} shows a wider region of the same data, on 24 December 2024, in the equatorial Pacific. 
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{figure}[h!]
\centerline{\includegraphics[width=\textwidth]{FIGS_CH_GROUPS/std_H2_maps.png}}
%\vspace{3.64in}
  \caption{ left: Variability of  the total $H_{L=4.2\mathrm{km}}$  on track 26 of cyclz 26 (24 December 2024) provided by in the Level 2 SWOT data using the method of \cite{Bohe&al.2025}. Center:  values of wave heights $H_{L=5\mathrm{km},T > 18 s}$ for the long waves using the method of \cite{Ardhuin&al.2024}, showing maxima at the same location as those in the total wave height. Right: estimate of the heights of the "short" waves  $H_{L=4.2\mathrm{km},T < 18 s}$ using  eq. (\ref{eq:HLsum}) with $L=4.2\mathrm{km}$. These long waves were generated by storm Eddie \citep{Ardhuin&al.2025}.}
\label{fig:groups_SWOT2}
\end{figure}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
The first result is that the variability of $H^2_{L=5\mathrm{km},T > 18 s}$ is of the order of 50\% of $H^2_{T > 18 s}$ because the spectra for the long waves are very narrow with a corresponding $Q_{kk,T > 18 s}$ around 250~m. Using the number of degrees of freedom $\nu_{kk,H}$ 
given by eq. (\ref{eq:nukk}) and the fact that  $H^2_{L}$ follows a $\chi^2$ distribution, gives an expected ratio $ 2 \pi  \sqrt{2}  Q_{kk,T > 18 s}  /L$  of the standard deviation of $H^2$ and the mean of $H^2$. This ratio is equal to 0.44 for $L=5$~km and 
$Q_{kk,T > 18 s} = 250$~m. The associated variance of $H^2_{L=5\mathrm{km},T > 18 s}$  when multiplied by the ratio of the grid resolutions $5^2/4.2^2$ is of the order 80\% of the variance of $H_{L=4.2\mathrm{km}}$  for latitudes in the range 
-2 to +2 degrees around the equator: in this example the strong fluctuations in SWH is thus explained by the presence of very long wave groups. This property could also be used to constrain the shape of the wave spectrum using fluctuations in SWH estimates. 


\section{Example of application: Along-track variability in altimeter data}
A practical important application of the result for spatial maps is the interpretation of the most common measurement of wave heights from space, using satellite altimeters. Looking at data from storm Dennis in the North Atlantic in 2020, as illustrated in Fig. \ref{figure:groups_storm1}, \cite{DeCarlo&al.2023} were surprised to find that for the same mean wave height of 9.3~m, the CFOSAT altimeter beam gave a very small variability on the north side, and a twice larger variability on the south side. What is this variability telling us about the sea state? 
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{figure}[h!]
...
\end{eqnarray}
with the peakedness $Q_{kk}$ given by eq. (\ref{eq:Qkk}). 
